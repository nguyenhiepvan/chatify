 ...
 *-------------------------------------------------------------
 */
var messenger, typingTimeout, typingNow = 0, temporaryMsgId = 0, defaultAvatarInSettings = null, messengerColor,
    dark_mode, messages_page = 1;

const messagesContainer = $(".messenger-messagingView .m-body"),
    messengerTitleDefault = $(".messenger-headTitle").text(), messageInput = $("#message-form .m-send"),
    auth_id = $("meta[name=url]").attr("data-user"), url = $("meta[name=url]").attr("content"),
...
function loadingWithContainer(className) {
    return `<div class="${className}" style="text-align:center;padding:15px">${loadingSVG("25px", "", "margin:auto")}</div>`;
}
...
let app_modal = function ({
                              show = true, name, data = 0, buttons = true, header = null, body = null,
                          }) {
    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                // Star status
                data.favorite > 0 ? $(".add-to-favorite").addClass("favorite") : $(".add-to-favorite").removeClass("favorite");
...
function fetchMessages(id, type, newFetch = false) {
    ...
    if (messenger != 0 && !noMoreMessages && !messagesLoading) {
        ...
        $.ajax({
            url: url + "/fetchMessages", method: "POST", data: {
                _token: access_token, id: id, type: type, page: messagesPage,
...
channel.bind("client-contactItem", function (data) {
    if (data.update_for == auth_id) {
        data.updating == true ? updateContactItem(data.update_to) : console.error("[Contact Item updates] Updating failed!");
    }
});
...
function updateContactItem(user_id) {
    if (user_id != auth_id) {
        ...
        $.ajax({
            url: url + "/updateContacts", method: "POST", data: {
                _token: access_token, user_id,
            }, dataType: "JSON", success: (data) => {
                const totalContacts = $(".listOfContacts").find(".messenger-list-item")?.length || 0;
                if (totalContacts < 1) $(".listOfContacts").find(".message-hint").remove();
                listItem.remove();
...
function getFavoritesList() {
    ...
    $.ajax({
        url: url + "/favorites", method: "POST", data: {_token: access_token}, dataType: "JSON", success: (data) => {
            if (data.count > 0) {
...
function deleteConversation(id) {
    $.ajax({
        ...
        beforeSend: () => {
            // hide delete modal
            app_modal({
                show: false, name: "delete",
            });
            // Show waiting alert modal
            app_modal({
                show: true, name: "alert", buttons: false, body: loadingSVG("32px", null, "margin:auto"),
            });
        },
        success: (data) => {
            ...
            if (!data.deleted) console.error("Error occurred, messages can not be deleted!");

            // Hide waiting alert modal
            app_modal({
                show: false, name: "alert", buttons: true, body: "",
            });
        },
        error: () => {
            console.error("Server error, check your response");
    ...
    $.ajax({
        ...
        success: (data) => {
            if (data.error) {
                ...
                app_modal({
                    show: true, name: "alert", buttons: true, body: data.msg,
                });
            } else {
                // Hide alert modal
                app_modal({
...
$(document).ready(function () {
    ...
    function attachmentValidate(file) {
        ...
        if (!getAllowedExtensions.includes(fileExtension.toString().toLowerCase())) {
    ...
    $("#message-form .m-send").on("keydown", () => {
        if (typingNow < 1) {
            ...
            triggered ? console.info("[+] Triggered") : console.error("[+] Not triggered");
            // Typing now
            typingNow = 1;
        }
        // Clear typing timeout
    ...
    $(".messenger-search").on("keyup", function (e) {
        $.trim($(this).val()).length > 0 ? $(".messenger-search").trigger("focus") + messengerSearch($(this).val()) : $(".messenger-tab").hide() + $('.messenger-listView-tabs a[data-view="users"]').trigger("click");
    });
